...
router.post('/cambiandoPassword', async( req, res)=>{
    ...
    const updateEmpleado = await empleadoTemlo.updateOne({cedulaE:req.body.user},{$set:{editado:false, contrasenaE:newPassSecure, conCelular:false, conMail:false}})
...
router.post('/iniSesion', async( req, res)=>{
    ...
    if (!empleadoTem){
    ...
    }else if(empleadoTem.empresa.nit!=req.body.nitEmpresa){
        return res.json({
            title: 'Error en nit',
            error: 'Nit erroneo',
            success:false,
        ...
        const editado = empleadoTem.editado
        let conQ = ''
        if(empleadoTem.conCelular){
            conQ = 'Celular'
        }
        if(empleadoTem.conMail){
            conQ = 'Email'
        }
        return res.json({
        ...
        if(empleadoTem.editado==false){
            ...
            if(req.body.sms){
                ...
                let okEnvio = await maill(pass,empleadoTem.emailE)
                if(okEnvio){
                    const coded = crypto.AES.encrypt(pass, contras).toString()
                    res.json({ 
                        success:true,
            ...
            res.json({ 
                conQ,
                usuario:empleadoTem.nombreE+' '+empleadoTem.primerApellidoE+' '+empleadoTem.segundoApellidoE,
                success:true,
                status:201
            })
        } 
...
router.post('/recuperarContrasenaCel', async( req, res)=>{
    const empleado = await empleadoTemlo.findOne({cedulaE:parseInt(req.body.codigo)}).populate('empresa')
        if(empleado){
            if(empleado.empresa.nit!=req.body.nitEmpresa){
                res.json({
...
router.post('/loginEmpleadoTemlo', async (req, res)=>{
    ...
    const empleado = await empleadoTemlo.findOne({cedulaE:req.body.user})
    if(empleado){
...
router.post('/actualizarToken',authe, async (req, res)=>{
    ...
    if (token) {
        ...
        jwt.verify(token.replace(/^Bearer\s/, ''), config.authSecret,async (err, decoded)=>{
            ...
            if(decoded) {       
                const _id=decoded._id
                let userr = await empleadoTemlo.findOne({'_id':_id})
                const payload = {
                    _id:userr._id,
                }
                    const token = jwt.sign(payload,config.authSecret,{
